...
class GoBoardUtil(object):
    ...
    def generate_all_policy_moves(board,pattern,check_selfatari):
        ...
        for move in legal_moves:
            y = GoBoardUtil.filleye_filter(board, move, board.current_player)
            #y= GoBoardUtil.selfatari_filter(board, move, 2-board.current_player)
            if(y==None):
                temp_list.append(move)



        legal_moves = temp_list

        proptotal = 0
        for move in legal_moves:
            x= Feature.find_move_feature(board, move)
            total = 1

            for value in x:
                total = total * Features_weight[value]

            proptotal = total + proptotal
            #print(move, total)
        ...
        for move in legal_moves:
            ...
            for value in x:
                total = total * Features_weight[value]
            move_list.append([move, total/proptotal])



        #for item in MC_tree.get_children():
        #    move=item
        #    prob=MC_tree.get_move_prob()
        #    moves.append([move, prob])
        move_list = sorted(move_list, key = getKey, reverse = True)
        return move_list
    ...
    def generate_max_move(board, use_pattern, check_selfatari):
        #from mcts import MCTS
        from Go6 import Go6Player
        moves = []

        moves = GoBoardUtil.generate_all_policy_moves(board,use_pattern,check_selfatari)
    ...
    def generate_rand_max(board, use_pattern, check_selfatari):
        #from mcts import MCTS
        from Go6 import Go6Player
        moves = []
