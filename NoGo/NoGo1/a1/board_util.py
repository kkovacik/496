...
# Added Error Messages for each of the errors 

def raise_capture_error():
    raise ValueError("capture")
    return 
def raise_suicide_error():
...
class GoBoardUtil(object):
    ...
    ### Prints error message beside other ones here ###
    @staticmethod
    def move_to_coord(point, board_size):
        ...
        try:
            ...
            raise ValueError("%s wrong number of arguments" % s)
        if not (col <= board_size and row <= board_size):
            raise ValueError("wrong coordinate")
	#if not (capture_check()==False):
	#    raise ValueError("capture")
	#    board.lm_reset()
